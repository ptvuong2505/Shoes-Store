...
public class ProductService : IProductService
{
    ...
    public async Task<AdminProductItemDto> UpdateAdminProductAsync(string id, AdminUpsertProductRequest request)
    {
        ...
        {
            throw new NotFoundException(ErrorCodes.InvalidProductId, "Invalid product ID format.");
        }
    ...
    public async Task DeleteAdminProductAsync(string id)
    {
        if (!Guid.TryParse(id, out var productId))
        {
            throw new NotFoundException(ErrorCodes.InvalidProductId, "Invalid product ID format.");
    ...
    public async Task<FiltersDto> GetFiltersAsync()
    {
        ...
        return new FiltersDto(genders, brands, sizes);
    }
    ...
    public async Task<ProductDetailDto> GetProductDetailAsync(string id)
    {
        ...
        var productDetail = await _context.Products.Where(p => p.Id == guid)
            .Select(p => new
            {
                ...
                Sizes = p.Inventories.Select(i => new ProductSizeDto(i.Size.Value, i.Quantity)).ToList(),
                RatingGroups = p.Reviews.GroupBy(r => r.Rating).Select(g => new { Rating = g.Key, Count = g.Count() }).ToList(),
                Reviews = p.Reviews.Select(r => new ReviewDto(
                    r.Id,
                    _context.Users.Where(u => u.Id == r.UserId).Select(u => u.UserName ?? u.Email ?? "Unknown").FirstOrDefault() ?? "Unknown",
                    r.Rating,
                    r.Comment,
                    r.CreatedAt
                )).ToList()
